...
- **GitHub access:** `GITHUB_TOKEN` is only set if the user opted in via `.env` — check `printenv GITHUB_TOKEN` before assuming. When present, embed it in the URL as `https://x-access-token:${GITHUB_TOKEN}@github.com/owner/repo` for both read (clone, fetch, ls-remote) and write (push), scoped to whatever the user granted the token. If it's not set, don't attempt to push or access private repos — tell the user to either set `GITHUB_TOKEN` in `.env` (README has the walkthrough) or push from the host.
