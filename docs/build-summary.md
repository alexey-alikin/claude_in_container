...
| Path | Purpose |
| --- | --- |
| `Dockerfile` | `node:20-slim` + `apt-get upgrade` for Debian security patches + `ca-certificates`, `curl`, `git` (HTTPS git remotes need the CA bundle; `curl` for hitting APIs and debugging connectivity), and `python3` with `pip`/`venv` (PyPI already allowed in the hardened egress list; runtime installs use a venv under `/workspace` since the rootfs is read-only and the global env is PEP 668 externally-managed) + global install of `@anthropic-ai/claude-code` (at `latest`) + `uv`/`uvx` copied from `ghcr.io/astral-sh/uv:latest` (lets Claude run Python-based MCP servers; `node`/`npx` already cover Node-based ones); creates non-root `claudeuser` whose UID matches the host via the `USER_UID` build arg. |
...
| `claude.sh` | Wrapper script. Subcommands: `new <name>`, `shell <name>`, `run <name> -- <args>`, `build [<args>]`, `list`, `help`. Validates project names against `[a-zA-Z0-9_-]+`. Honors `HARDENED=1` env var to also apply the overlay. On first invocation of `build`/`shell`/`run`/`new`, copies the host's `git config --global user.{name,email}` into `claude_home/.gitconfig.local` so commits inside the container inherit the same identity (skip with `CIC_SKIP_GIT_IDENTITY=1`). |
...
- `@anthropic-ai/claude-code` is not version-pinned (decision: accept `latest` drift, document the manual pin step in README troubleshooting if it ever breaks). The `uv`/`uvx` image (`ghcr.io/astral-sh/uv:latest`) is unpinned for the same reason; `SECURITY.md` documents how to pin it.
